...
router.post('/items/:id/media', auth, uploadSingle.single('media'), async (req, res) => {
  ...
  try {
    ...
    if(item.media.length >= 3) {
      return res.status(409).send({ error: 'You cannot upload more than 3 images per item' })
    }
...
router.get('/items/user/:id', async (req, res) => {
  try {
    ...
    if(!user) {
      return res.status(404).send({error: `Could not find user with id: ${req.params.id}`})
    }
...
router.delete('/items/:itemId/media/:imageId', auth, async (req, res) => {
  ...
  try {
    ...
    item.media = item.media.filter(image => String(image._id) !== String(imageId))
